...
<tr class="ui-widget-header"><form id="filters" method="get">
    ...
    <a href="${h.url.current(sortname=field.name, sortorder='asc')}" title="A-Z"><span class="ui-icon ui-icon-circle-arrow-n" style="float: right;"></span></a>
...
%>
        <div>${html}</div>
        <a href="${h.url.current(sortname=field.name, sortorder='desc')}" title="Z-A"><span class="ui-icon ui-icon-circle-arrow-s" style="float: right;"></span></a>
        </th>
        %endfor
