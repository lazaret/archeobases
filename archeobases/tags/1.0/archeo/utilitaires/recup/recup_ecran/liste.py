...
#
#
# $Id: liste.py,v 1.4 2001/07/05 15:29:15 rachel Exp $
#
# $Log: liste.py,v $
# Revision 1.4  2001/07/05 15:29:15  rachel
# plein de modifs
#
# Revision 1.3  2001/03/20 19:59:36  jerome
...
liste =['A','B','P','AF','BF','PF','FL','BT','BC','BR','BD','BTA','BTC','BTP','BTE','BTDC','BTDN','BTT','BTPI','BTPC','BTPM','BTPT','BTPR','M'] ###OK
for i in range (0,len(liste)):
...
liste =['1','2A','2B','2C','2D','2E','2F','3'] ####OK
for i in range (0,len(liste)):
        insert = "INSERT INTO controle_o_etat (o_etat) VALUES (" + db.quote(string.upper(liste[i]), "text") + ");"
        db.query(insert)

liste =['IND','ANGPROX','LATGT','LATGPROX','LATGMES','LATGDIS','ANGGDIS','TRDIST','TRDISG','TRDISMES','TRDISD','PTDIS','ANGDDIS','LATDT','LATDDIS','LATDMES','LATDPROX','ANGDPROX','TRPROXT','TRPROXD','TRPROXMES','TRPROXG','PTPROX','ARMEDT','ARMEDPROX','ARMEDMES','ARMEDDIS','PERIP'] ###OK
for i in range (0,len(liste)):
        insert = "INSERT INTO controle_o_localisation (o_localisation) VALUES (" + db.quote(string.upper(liste[i]), "text") + ");"
        db.query(insert)

liste =['DIR','INV','MIX','MIXAL','PROFL','PROFT','PROFM','MSG','MSD','MSX','MIG','MID'] ###OK
for i in range (0,len(liste)):
...
liste =['REC','CONV','CONC','SCONV','SCONC'] ####OK
for i in range (0,len(liste)):
