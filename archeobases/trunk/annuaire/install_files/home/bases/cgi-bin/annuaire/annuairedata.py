    ...
    def __verify_mandatory__(self):
        ...
        for champ in self.__champs__.keys():
            try:
                ...
                annuaireconf.fatalerror_message("Erreur sur la vérification du champ obligatoire %s : %s [%s]" % (champ, msg, self.__form__[champ]))
        return not_ok
...
#########################################################################
    def __menugenerique__(self, enreg, penreg=None):
        self.__doc__.push()
        self.__doc__.td(bgcolor=annuaireconf.basform_bgcolorright, valign="top", align="center")
...
#################################################################################
    def formulaire_hook(self, enreg=None, current=0, maximum=0, penreg=None):
        ...
        if enreg:
            ...
            self.__doc__.font(size = annuaireconf.font_size)
            if maximum:
                self.__doc__.a("Premiere", href="#anchor_" + self.__tablename__ + "0")
                self.__doc__.insert_text("&nbsp;" * 5)
                if current:
                    self.__doc__.a("Precedente", href="#anchor_" + self.__tablename__ + `current - 1`)
                else:
                    self.__doc__.insert_text("Precedente")
                self.__doc__.insert_text("&nbsp;" * 5)
            self.__doc__.push()
            ...
            if maximum:
                ...
                self.__doc__.insert_text("&nbsp;" * 5)
                self.__doc__.a("Derniere", href="#anchor_" + self.__tablename__ + `maximum`)
            self.__doc__.br()
            self.__doc__.font(size = annuaireconf.font_size)
            if hasattr(self, "__listeparents__"):
                ...
                for p in self.__listeparents__:
                    ...
                    if p == "hachereau":
                        ...
                        self.dessine_lien(("Galets aménagés"), annuaireconf.script_location("mod" + p) + "?" + urllib.urlencode(dico), annuaireconf.lien_parent_bgcolor)
                        ...
                        self.dessine_lien((string.capitalize(p)), annuaireconf.script_location("mod" + p) + "?" + urllib.urlencode(dico), annuaireconf.lien_parent_bgcolor)
            ...
            if hasattr(self, "__listenfants__"):
                ...
                for enfant in self.__listenfants__:
                    ...
                    if enfant == "hachereau":
                        ...
                        self.dessine_lien(("Mesure des Os"), annuaireconf.script_location("mod" + enfant) + "?" +urllib.urlencode(dico), annuaireconf.lien_enfant_bgcolor)
                    elif enfant == "fracture_faune":
                        self.dessine_lien(("Fractures faunes"), annuaireconf.script_location("mod" + enfant) + "?" + urllib.urlencode(dico), annuaireconf.lien_enfant_bgcolor)
                    elif enfant == "nucleus":
                        ...
                        self.dessine_lien(("Galets aménagés"), annuaireconf.script_location("mod" + enfant) + "?" + urllib.urlencode(dico), annuaireconf.lien_enfant_bgcolor)
                    else:
                        pluriel = ''
                        if enfant[-1] != 's':
                            pluriel = 's'
                            self.dessine_lien((string.capitalize(enfant) + pluriel), annuaireconf.script_location("mod" + enfant) + "?" + urllib.urlencode(dico), annuaireconf.lien_enfant_bgcolor)
            self.__doc__.pop()
        self.__doc__.push()
        self.__doc__.tr()
        self.__doc__.form(method = "POST", action = annuaireconf.script_location("mod" + self.__tablename__))
        ...
        #####################COMPTER################
            ...
            if not lg:
                ...
                if self.__parent__ == self.__tablename__:
                    ...
                    self.__doc__.font(" enregistrement(s) trouve(s)", color="red")
                    self.__doc__.pop()
            self.formulaire_hook(penreg=penreg)
            ...
            if self.__form__.has_key("action"):
                ...
                if len(enregs):
                    ...
                    annuaireconf.log_message("Tentative d'accès à un enregistrement d'identifiant inexistant")
                    self.formulaire_hook(penreg = penreg)
        else:
            #FIXME je me demande ce qui reste execute dans ce qui est ci-dessous
...
############################### RECUPERE VALEUR #####################################
    def recupere_valeur(self, nom_table, nom_champ, liste_clefs, enreg):
        if enreg:
            query = "SELECT " + nom_champ + " FROM " + nom_table + " WHERE identifiant=%s ;" %( self.__db__.quote(enreg["identifiant"], "decimal"))
...
################################ CHAMP LISTE TABLE ##############################
    def champ_liste_table(self, nom_table, liste_clefs, nom_champ, libelle, enreg, penreg, titre, dontchange) :
        ...
        self.__doc__.font(libelle, size=annuaireconf.font_size)
        self.__doc__.font(size=annuaireconf.font_size)
        self.__doc__.pop()
        self.__doc__.push()
    ...
    def formulaire_hook_parametre(self, enreg=None, current=0, maximum=0, penreg=None):
        ...
        if enreg:
            ...
            if self.__vraiparent__ != None:
                ...
                for clef in self.__clefsparent__():
                    dico[clef] = enreg[clef]
                self.__doc__.a(string.capitalize(self.__vraiparent__), href = annuaireconf.script_location("mod" + self.__vraiparent__) + "?" + urllib.urlencode(dico))
            if hasattr(self, "__listenfants__"):
                dico = {"action": "Chercher"}
                for clef in self.__listeclefs__:
                    dico[clef] = enreg[clef]
                for enfant in self.__listenfants__:
                    self.__doc__.insert_text("&nbsp;" * 5)
                    self.__doc__.a(string.capitalize(enfant) + 's', href=annuaireconf.script_location("mod" + enfant) + "?" + urllib.urlencode(dico))
            self.__doc__.pop()
        self.__doc__.push()
        self.__doc__.tr()
        self.__doc__.form(method = "POST", action = annuaireconf.script_location("mod" + self.__tablename__))
