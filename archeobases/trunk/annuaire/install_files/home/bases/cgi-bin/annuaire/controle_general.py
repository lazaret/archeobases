...
if form.has_key("champ"):
    ...
    doc.ecran_requetes(annuaireconf.bas_bgcolor, annuaireconf.bas_bgcolor, annuaireconf.bas_bgcolor, annuaireconf.bas_bgcolor, annuaireconf.bas_bgcolor, param)
else:
    s = "Contenu du formulaire:\n"
    for k in form.keys():
        s = s + "%s = %s\n" % (k, form[k].value)
    annuaireconf.fatalerror_message(s)
doc.output()
