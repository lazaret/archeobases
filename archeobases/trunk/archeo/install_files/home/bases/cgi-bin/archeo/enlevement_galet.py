...
class Enlevement_Galet(archeodata.Data) :
        ...
        def ga_ordre_base_to_form(self, enreg, penreg = None) :
                ...
                #aicheclefs.display_x_ordre(self, "ga_ordre", 1, 15, enreg, penreg, change=0)
...
##############################################################################################################################
        ...
        def modifier(self) :
                ...
                self.__db__.query(self.make_update_query(["zone", "numero", "bis", "ga_ordre","eg_rang"]))
                return 0

        def supprimer(self) :
 
                # on efface l' enlevement
                self.delete_records(["zone", "numero", "bis", "ga_ordre", "eg_rang"])
                return 0

        def creer(self) :
                # si l' enlevement existe, on refuse de le creer
                if self.exist(["zone", "numero", "bis", "ga_ordre", "eg_rang"]) :
