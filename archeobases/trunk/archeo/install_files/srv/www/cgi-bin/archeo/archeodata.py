    ...
    def __verify_mandatory__(self):
        ...
        for champ in self.__champs__.keys():
            try:
                ...
                archeoconf.fatalerror_message("Erreur sur la vérification du champ obligatoire %s: %s [%s]" % (champ, msg, self.__form__[champ]))
        return not_ok
...
###########################################################################
    ...
    def exist(self, champs, table=None):
        """ Teste si un enregistrement existe avec la valeur value pour le champ field,
            renvoie le nbre d'enregistrements concernes, ou 0 sinon"""
        if table == None:
            table = self.__tablename__
            ...
            if hasattr(self, "__listeparents__"):
                ...
                for p in self.__listeparents__:
                    ...
                    if p == "hachereau":
                        ...
                        self.dessine_lien(("Galets aménagés"), archeoconf.script_location("mod" + p) + "?" + urllib.urlencode(dico), archeoconf.lien_parent_bgcolor)
            ...
            if hasattr(self, "__listenfants__"):
                ...
                for enfant in self.__listenfants__:
                    ...
                    if enfant == "hachereau":
                        ...
                        self.dessine_lien(("Galets aménagés"), archeoconf.script_location("mod" + enfant) + "?" + urllib.urlencode(dico), archeoconf.lien_enfant_bgcolor)
                    else:
                        pluriel = ''
                        if enfant[-1] != 's':
                            pluriel = 's'
        ...
        #####################COMPTER################
            ...
            if self.__form__.has_key("action"):
                ...
                ################## CREER #############################
                    ...
                    if not self.__verify_mandatory__():
                        ...
                        if retour == -1:
                            ...
                            # erreur de faune ou d'industrie en fonction de la nature
                            self.__doc__.script('alert("Nature incorrecte ! Merci de vérifier.")')
                            self.__doc__.script('parent.bas.location = "' + archeoconf.script_location("modcarnet") + '?action=Chercher&' + self.__make_url__(primarykeys) + '"')
                        elif retour == 1:
                ...
                if len(enregs):
                    ...
                    archeoconf.log_message("Tentative d'accès à un enregistrement d'identifiant inexistant")
                    self.formulaire_hook(penreg=penreg)
        else:
            ### TODO je me demande ce qui reste execute dans ce qui est ci-dessous
