...
###########################################################################
    ...
    def exist(self, champs, table=None):
        """Teste si un enregistrement existe avec la valeur value pour le champ field, renvoie
        le nombre d'enregistrements concernes, ou 0 sinon"""
        if table == None:
            table = self.__tablename__
        ...
        if self.__form__.has_key("action") and (self.__form__["action"].value == "Chercher"):
            ...
            if self.__form__.has_key("action"):
                ...
                if len(enregs):
                    ...
                    begoconf.log_message("Tentative d'accès à un enregistrement d'identifiant inexistant")
                    self.formulaire_hook(penreg=penreg)
        else:
