...
if ruser in begoconf.superusers:
    ...
    if (not form.has_key("nomrequete")) and (not form.has_key("requete")):
        doc.ecran_requetes(begoconf.bas1_bgcolor, begoconf.bas1_bgcolor, begoconf.bas1_bgcolor, begoconf.bas1_bgcolor, begoconf.bas1_bgcolor)
    else:
        if form.has_key("nomrequete") and not form.has_key("lue"):
            ...
            quequette = string.strip(string.replace(form["requete"].value, "\r", ""))
            doc.ecran_requetes(begoconf.bas1_bgcolor, begoconf.bas1_bgcolor, begoconf.bas1_bgcolor, begoconf.bas1_bgcolor, begoconf.bas1_bgcolor, requete=quequette)
            ...
            if (resultat != None) and (type(resultat) != type(0)):
                ...
                if nbrecords:
                    liste_champs = resultat.listfields() # un tuple avec la liste des champs
                    dict_valeurs = resultat.dictresult() # un tuple de dictionnaires de données
                    liste_valeurs = resultat.getresult() # un tuple de listes de données
                    ...
                    if form["presentation"].value == telechargecsv:
                        ...
                        csv_file.close()
                        if not os.path.exists("/srv/www/default/bego/resultat_requete.csv"):
                            os.symlink("/tmp/resultat_requete.csv", "/srv/www/default/bego/resultat_requete.csv")
                        doc.set_redirect("http://"+hostname+"/bego/resultat_requete.csv")
                    ...
                    if form["presentation"].value == telechargexls:
                        ...
                        xls_sheet = xls_file.add_sheet("requete_"+begoconf.base_courante)
                        # defaut cell style
                        ...
                        if not os.path.exists("/srv/www/default/bego/resultat_requete.xls"):
                            os.symlink("/tmp/resultat_requete.xls", "/srv/www/default/bego/resultat_requete.xls")
                        doc.set_redirect("http://"+hostname+"/bego/resultat_requete.xls")
                    # display HTML results
                        ...
                        if form["presentation"].value == elabore:
                            ...
                            for champ in liste_champs:
                                doc.th(champ, bgcolor=begoconf.bas1_bgcolor)
                            doc.pop()
