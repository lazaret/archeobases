...
class Dessin(collectiondata.Data):
    #
    # le nom de la table
...
################################### En entrée ###########################
    ...
    def m_date_decouverte_base_to_form(self, enreg, penreg=None):
        self.__doc__.push()
        afficheclefs.champ_saisie(self, "m_date_decouverte", "Date Dessin", 4, 4, " ", enreg, penreg)
        self.__doc__.pop()
    ...
    def m_date_base_to_form(self, enreg, penreg=None):
        self.__doc__.push()
        afficheclefs.champ_saisie(self, "m_date", "Date Copie", 4, 4, " ", enreg, penreg)
        self.__doc__.pop()
...
##############################################################################
    ...
    def supprimer(self):
        # s'il existe des figures ou des photos sur ce dessin on refuse la suppression
        i = self.__form__["identifiant"].value
    ...
    def creer(self):
        ...
        if self.exist(self.__listeclefs__, table="materiel"):
            ...
            # on insère maintenant le dessin dans la base
            i = self.__form__["identifiant"].value
