...
class Photo(collectiondata.Data) :
        ...
        def supprimer(self) :
                ...
                small = collectionconf.image_fullname(fname + "s.jpeg")
                big = collectionconf.image_fullname(fname + ".jpeg")
                normale = collectionconf.image_fullname(fname + ".tiff")
                try :
                        os.unlink(small)
                        os.unlink(big)
                        os.unlink(normale)
        ...
        def creer(self) :
                ...
                try :
                        ...
                        rep = collectionconf.image_fullname(i)
                        if not os.path.isdir(rep) :
                                os.mkdir(rep, 0755)

                except OSError, msg:
                        collectionconf.fatalerror_message("Impossible de créer le répertoire [%s] ou l'un de ses composants: %s" % (rep, msg))
                ...
                try :
                        ...
                        t = "mogrify_" + t
                        self.transfere_fichier(normale, collectionconf.mogrify_normale)
                        self.transfere_fichier(small, collectionconf.mogrify_small)
                        self.transfere_fichier(big, getattr(collectionconf, t))
                except :
                        collectionconf.fatalerror_message("Impossible de transférer la photo " + fname)
