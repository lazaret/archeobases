...
class Releve(collectiondata.Data) :
        #
        # le nom de la table
        ...
        __champs__ = {
                        ...
                        "m_type_materiel"       : { "type" : "text", "default" : "RELEVE",  "mandatory" : 1 , "longueur" : 0 , "memory" : 1 }, \
...
################################### En entrée ###########################
        ...
        def m_date_decouverte_base_to_form(self, enreg, penreg = None) :
                self.__doc__.push()
                afficheclefs.champ_saisie(self, "m_date_decouverte", "Date Releve", 4, 4, " ", enreg, penreg)
                self.__doc__.pop()
...
##############################################################################
        ...
        def supprimer(self) :
               # s'il existe des figures ou des photos sur ce releve on refuse la suppression
        ...
        def creer(self) :
                # si le releve n'existe pas déjà alors on la crée, sinon on refuse
                if self.exist(["identifiant"], table = "materiel") :
                        primarykeys = { "identifiant": None}
                        return (-1, primarykeys)
                else :
                        # on insère maintenant le releve dans la base
                        i = self.__form__["identifiant"].value
