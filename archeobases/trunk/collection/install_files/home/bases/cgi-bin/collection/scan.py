...
################################### En entrée ###########################
    ...
    def m_scan_epaisseur_coupe_base_to_form(self, enreg, penreg=None):
        self.__doc__.push()
        afficheclefs.champ_saisie(self, "m_scan_epaisseur_coupe", "Epaisseur coupe", 12, 12, " ", enreg, penreg)
        self.__doc__.pop()

    def m_scan_nb_coupes_base_to_form(self, enreg, penreg=None):
        self.__doc__.push()
        afficheclefs.champ_saisie(self, "m_scan_nb_coupes", "Nb. de coupes", 15, 15, " ", enreg, penreg)
        self.__doc__.pop()
...
##############################################################################
    ...
    def supprimer(self):
        # s'il existe des figures ou des photos sur ce scan on refuse la suppression
        i = self.__form__["identifiant"].value
    ...
    def creer(self):
        ...
        if self.exist(self.__listeclefs__, table="materiel"):
            ...
            # on insère maintenant le scan dans la base
            i = self.__form__["identifiant"].value
